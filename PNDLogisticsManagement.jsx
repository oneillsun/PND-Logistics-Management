...
const EC = { expired:{bg:"#2a0d0d",text:"#ff5555",border:"#7a2020"}, warning:{bg:"#2a1a00",text:"#ffaa00",border:"#7a4400"}, ok:{bg:"#0a1a0a",text:"#00cc66",border:"#1a5a2a"}, none:{bg:"#1a1a2a",text:"#555",border:"#2a2a3a"} };
...
const IS = { width:"100%", background:"#12121e", border:"1px solid #252538", borderRadius:6, padding:"9px 12px", color:"#e0e0f0", fontSize:14, fontFamily:"'DM Mono',monospace", outline:"none", boxSizing:"border-box" };
const B = (v="primary") => ({ padding:"9px 18px", borderRadius:6, border:"none", cursor:"pointer", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:14, letterSpacing:.5, background:v==="primary"?"#ff6200":v==="success"?"#00a050":v==="danger"?"#cc2000":"#1e1e30", color:v==="ghost"?"#888":"#fff" });
...
{toasts.map(t=><div key={t.id} style={{background:t.type==="success"?"#0a2a18":t.type==="warn"?"#2a1e00":"#14142a",border:`1px solid ${t.type==="success"?"#00aa55":t.type==="warn"?"#aa8800":"#3a3a6a"}`,color:t.type==="success"?"#00ee77":t.type==="warn"?"#ffcc44":"#9999ff",padding:"10px 16px",borderRadius:8,fontSize:13,fontFamily:"'DM Mono',monospace",maxWidth:330,animation:"slideIn .2s ease"}}>{t.message}</div>)}

  </div>;
}

function Modal({title,onClose,children,wide}) {
return <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}>
<div onClick={e=>e.stopPropagation()} style={{background:"#0d0d1a",border:"1px solid #25253a",borderRadius:12,padding:26,width:"100%",maxWidth:wide?740:540,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 80px rgba(0,0,0,.95)"}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
<h2 style={{margin:0,fontSize:19,color:"#e0e0f0",fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:1}}>{title}</h2>
<button onClick={onClose} style={{background:"none",border:"none",color:"#555",cursor:"pointer",padding:4}}><Ico n="x" s={17}/></button>
</div>
{children}
</div>
...
<label style={{display:"block",fontSize:10,color:"#555",fontFamily:"'DM Mono',monospace",letterSpacing:1.2,marginBottom:5,textTransform:"uppercase"}}>{label}</label>
{children}
...
const c=SC[status]||{bg:"#1a1a1a",text:"#777",border:"#333"};
return <span style={{background:c.bg,color:c.text,border:`1px solid ${c.border}`,padding:"2px 10px",borderRadius:99,fontSize:11,fontFamily:"'DM Mono',monospace",whiteSpace:"nowrap"}}>{status}</span>;
}

function TInfo({tk}) {
const t=TERMINAL_DATA[tk]; if(!t) return null;
return <div style={{background:"#0a1828",border:"1px solid #1a3a5a",borderRadius:8,padding:"10px 14px",marginBottom:14}}>
<div style={{display:"flex",alignItems:"flex-start",gap:7,marginBottom:5}}><span style={{color:"#3a8aff",flexShrink:0,marginTop:1}}><Ico n="pin" s={12}/></span><span style={{fontSize:12,color:"#6aaddd",fontFamily:"'DM Mono',monospace",lineHeight:1.5}}>{t.address}</span></div>
<div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
<div style={{display:"flex",alignItems:"center",gap:5}}><span style={{color:"#3a8aff"}}><Ico n="user" s={11}/></span><span style={{fontSize:12,color:"#6aaddd",fontFamily:"'DM Mono',monospace"}}>{t.manager}</span></div>
<div style={{display:"flex",alignItems:"center",gap:5}}><span style={{color:"#3a8aff"}}><Ico n="phone" s={11}/></span><span style={{fontSize:12,color:"#6aaddd",fontFamily:"'DM Mono',monospace"}}>{t.phone}</span></div>
</div>
...
return <Modal title="Send Candidate Notification" onClose={onClose} wide>
<div style={{background:"#070f07",border:"1px solid #1a5a1a",borderRadius:10,padding:18}}>
<div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16,background:"#0a1a0a",border:"1px solid #1a4a1a",borderRadius:8,padding:"10px 14px"}}>
...
<div><div style={{fontSize:17,color:"#e0e0f0",fontWeight:700,fontFamily:"'Barlow Condensed',sans-serif"}}>{test.candidateName}</div><div style={{fontSize:13,color:"#00cc66",fontFamily:"'DM Mono',monospace"}}>{test.phone}</div></div>
...
<Ico n="sms" s={26}/><span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:16,color:"#fff"}}>TEXT MESSAGE</span><span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(255,255,255,.6)"}}>Opens Messages app</span>
</button>
...
<span style={{fontSize:26}}>💬</span><span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:16,color:"#44ee88"}}>WHATSAPP</span><span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"rgba(68,238,136,.5)"}}>Opens WhatsApp</span>
</button>
</div>
...
<button onClick={copy} style={{background:copied?"#0a2a18":"#1e1e30",border:`1px solid ${copied?"#00aa55":"#252538"}`,color:copied?"#00ee77":"#888",padding:"8px 14px",borderRadius:6,cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:600,fontSize:13,display:"flex",alignItems:"center",gap:5}}>
<Ico n="copy" s={13}/>{copied?"✅ Copied!":"Copy Text"}
</button>
<button onClick={onClose} style={{marginLeft:"auto",background:"none",border:"none",color:"#555",cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:600,fontSize:13}}>Done</button>
</div>
</div>
</Modal>;
}
...
{prev&&<div style={{background:"#030a03",border:"1px solid #1a5a1a",borderRadius:8,padding:14,marginBottom:14}}><div style={{fontSize:10,color:"#2a7a2a",fontFamily:"'DM Mono',monospace",marginBottom:8}}>SMS PREVIEW</div><pre style={{margin:0,fontFamily:"'DM Mono',monospace",fontSize:11,color:"#77bb88",lineHeight:1.7,whiteSpace:"pre-wrap"}}>{buildSms(form)}</pre></div>}
...
{!existing&&<button style={{background:"none",border:"1px solid #1a5a1a",color:"#44dd77",padding:"8px 14px",borderRadius:6,cursor:"pointer",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:13,display:"flex",alignItems:"center",gap:5}} onClick={()=>setPrev(p=>!p)}><Ico n="sms" s={13}/>{prev?"Hide":"Preview SMS"}</button>}
...
return <>
<div style={{background:"#12121e",border:"1px solid #25253a",borderRadius:8,padding:14,marginBottom:14}}>
<div style={{fontSize:10,color:"#444",fontFamily:"'DM Mono',monospace",textTransform:"uppercase"}}>Candidate</div>
<div style={{fontSize:20,color:"#e0e0f0",fontWeight:700,marginTop:4,fontFamily:"'Barlow Condensed',sans-serif"}}>{test.candidateName}</div>
<div style={{fontSize:12,color:"#555",marginTop:2,fontFamily:"'DM Mono',monospace"}}>{test.fedexId} · {test.phone}</div>
</div>
...
return <div style={{background:"#0b0b18",border:`1px solid ${needsOutcome?"#ff6200":"#1e1e30"}`,borderRadius:10,padding:16,boxShadow:needsOutcome?"0 0 0 1px #ff6200,0 0 20px rgba(255,98,0,.1)":"none"}}>
{needsOutcome&&<div style={{background:"#2a1200",border:"1px solid #ff6200",borderRadius:6,padding:"6px 10px",marginBottom:10,display:"flex",alignItems:"center",gap:6,animation:"pulse 2s infinite"}}><Ico n="bell" s={13}/><span style={{fontSize:12,color:"#ff9944",fontFamily:"'DM Mono',monospace"}}>Road test ended - awaiting outcome</span></div>}
<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
<div><div style={{fontSize:17,fontWeight:700,color:"#e0e0f0",fontFamily:"'Barlow Condensed',sans-serif"}}>{test.candidateName}</div><div style={{fontSize:11,color:"#3a3a5a",fontFamily:"'DM Mono',monospace"}}>FX ID: {test.fedexId}</div></div>
<Badge status={test.status}/>
</div>
<div style={{fontSize:12,color:"#666",fontFamily:"'DM Mono',monospace",display:"flex",flexDirection:"column",gap:3,marginBottom:10}}>
<div>📞 {test.phone}</div>
<div style={{color:"#4488aa"}}>📍 {test.terminal}</div>
<div style={{color:"#2a4a5a",paddingLeft:18}}>{t.address}</div>
...
<div style={{color:"#3a5566"}}>👤 {t.manager} · {t.phone}</div>
</div>
{test.status==="Passed"&&<div style={{background:"#071a0f",border:"1px solid #1a5a2a",borderRadius:6,padding:"6px 10px",marginBottom:10,fontSize:12,color:"#00dd66",fontFamily:"'DM Mono',monospace"}}>✅ PASSED{test.firstDay?` · Training: ${new Date(test.firstDay+"T12:00:00").toLocaleDateString()}`:""}</div>}
{test.status==="Failed"&&<div style={{background:"#1a0707",border:"1px solid #6a1a1a",borderRadius:6,padding:"6px 10px",marginBottom:10,fontSize:12,color:"#ff5555",fontFamily:"'DM Mono',monospace"}}>❌ FAILED{test.feedback?` · ${test.feedback.slice(0,70)}`:""}</div>}
...
<button onClick={()=>onSms(test)} style={{background:"none",border:"1px solid #1a4a1a",color:"#44dd77",cursor:"pointer",padding:"5px 10px",borderRadius:5,fontSize:12,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:600,display:"flex",alignItems:"center",gap:4}}><Ico n="sms" s={12}/>SMS</button>
...
<button onClick={()=>onDelete(test.id)} style={{...B("ghost"),padding:"5px 8px",marginLeft:"auto",color:"#ff4444"}}><Ico n="trash" s={13}/></button>
</div>
...
<label style={{fontSize:10,color:"#555",fontFamily:"'DM Mono',monospace",letterSpacing:1.2,textTransform:"uppercase"}}>Uniform Items</label>
<button onClick={addItem} style={{background:"none",border:"1px solid #252538",color:"#888",cursor:"pointer",padding:"4px 10px",borderRadius:5,fontSize:12,display:"flex",alignItems:"center",gap:4,fontFamily:"'Barlow Condensed',sans-serif",fontWeight:600}}><Ico n="plus" s={12}/>Add Item</button>
</div>
<div style={{background:"#10101e",borderRadius:8,padding:"10px 12px"}}>
<div style={{display:"grid",gridTemplateColumns:"2fr 1fr 0.7fr auto",gap:6,marginBottom:6}}>
{["Item Type","Size","Qty",""].map((h,i)=><span key={i} style={{fontSize:10,color:"#3a3a5a",fontFamily:"'DM Mono',monospace",letterSpacing:1,textTransform:"uppercase"}}>{h}</span>)}
</div>
{form.items.map((item,i)=><div key={i} style={{display:"grid",gridTemplateColumns:"2fr 1fr 0.7fr auto",gap:6,marginBottom:6,alignItems:"center"}}>
...
{form.items.length>1?<button onClick={()=>remItem(i)} style={{background:"none",border:"none",color:"#ff4444",cursor:"pointer",padding:4}}><Ico n="x" s={13}/></button>:<span/>}
</div>)}
</div>
<div style={{fontSize:11,color:"#2a2a4a",fontFamily:"'DM Mono',monospace",marginTop:6}}>Pants & Shorts: waist sizes W24-W48. All others: XS-4XL.</div>
</div>
...
return <div style={{background:"#0b0b18",border:"1px solid #1e1e30",borderRadius:10,padding:16}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
<div><div style={{fontSize:15,fontWeight:700,color:"#e0e0f0",fontFamily:"'Barlow Condensed',sans-serif"}}>{req.terminal}</div><div style={{fontSize:11,color:"#3a3a5a",fontFamily:"'DM Mono',monospace",marginTop:1}}>By: {req.requestedBy}</div></div>
<Badge status={req.status}/>
</div>
{t.address&&<div style={{fontSize:11,color:"#2a3a45",fontFamily:"'DM Mono',monospace",marginBottom:10}}>📍 {t.address}</div>}
<div style={{background:"#0e0e1c",borderRadius:6,padding:"8px 10px",marginBottom:10}}>
{req.items.map((item,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#aaa",fontFamily:"'DM Mono',monospace",padding:"3px 0",borderBottom:i<req.items.length-1?"1px solid #1e1e30":"none"}}><span>{item.type}</span><span style={{color:"#666"}}>{BOTTOM_TYPES.includes(item.type)?`W${item.size}`:item.size} × {item.qty}</span></div>)}
<div style={{marginTop:6,paddingTop:6,borderTop:"1px solid #252538",display:"flex",justifyContent:"space-between",fontSize:11,color:"#555",fontFamily:"'DM Mono',monospace"}}><span>TOTAL PIECES</span><span style={{color:"#888"}}>{total}</span></div>
</div>
{req.notes&&<div style={{fontSize:11,color:"#3a4455",fontFamily:"'DM Mono',monospace",marginBottom:8,fontStyle:"italic"}}>{req.notes}</div>}
<div style={{fontSize:11,color:"#2a2a3a",fontFamily:"'DM Mono',monospace",marginBottom:10}}>Submitted {new Date(req.createdAt).toLocaleDateString()}</div>
<div style={{display:"flex",gap:6}}>
{req.status==="Pending"&&<><button onClick={()=>onEdit(req)} style={{...B("ghost"),padding:"5px 12px",fontSize:12}}>Edit</button><button onClick={()=>onFulfill(req.id)} style={{...B("success"),padding:"5px 12px",fontSize:12,display:"flex",alignItems:"center",gap:5}}><Ico n="check" s={12}/>Mark Fulfilled</button></>}
<button onClick={()=>onDelete(req.id)} style={{...B("ghost"),padding:"5px 8px",marginLeft:"auto",color:"#ff4444"}}><Ico n="trash" s={13}/></button>
</div>
...
return <div style={{background:c.bg,border:`1px solid ${c.border}`,borderRadius:7,padding:"8px 12px",flex:1}}>
<div style={{fontSize:10,color:c.text,fontFamily:"'DM Mono',monospace",textTransform:"uppercase",opacity:.7,marginBottom:3}}>{label}</div>
<div style={{fontSize:12,color:"#e0e0f0",fontFamily:"'DM Mono',monospace",marginBottom:2}}>{dateStr?new Date(dateStr+"T12:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):"Not set"}</div>
...
return <div style={{background:"#0b0b18",border:`1px solid ${hasAlert?(isExp?"#7a2020":"#7a4400"):"#1e1e30"}`,borderRadius:10,padding:16}}>
{hasAlert&&<div style={{background:isExp?"#2a0d0d":"#2a1a00",border:`1px solid ${isExp?"#ff5555":"#ffaa00"}`,borderRadius:6,padding:"6px 10px",marginBottom:10,display:"flex",alignItems:"center",gap:6,animation:"pulse 2s infinite"}}><Ico n="warn" s={13}/><span style={{fontSize:12,color:isExp?"#ff7777":"#ffcc44",fontFamily:"'DM Mono',monospace"}}>{isExp?"EXPIRED - action required":"Expiring within 30 days - renewal needed"}</span></div>}
<div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
<div><div style={{fontSize:20,fontWeight:800,color:"#e0e0f0",fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:1}}>🚚 {truck.truckNumber}</div><div style={{fontSize:12,color:"#4488aa",fontFamily:"'DM Mono',monospace"}}>{truck.terminal}</div><div style={{fontSize:11,color:"#2a3a45",fontFamily:"'DM Mono',monospace"}}>{t.address}</div></div>
<div style={{textAlign:"right"}}><div style={{fontSize:15,color:"#e0e0f0",fontFamily:"'DM Mono',monospace",fontWeight:700}}>{truck.licensePlate}</div>{truck.regState&&<div style={{fontSize:11,color:"#555",fontFamily:"'DM Mono',monospace"}}>State: {truck.regState.toUpperCase()}</div>}</div>
</div>
{truck.vin&&<div style={{fontSize:11,color:"#3a3a5a",fontFamily:"'DM Mono',monospace",marginBottom:10}}>VIN: {truck.vin}</div>}
<div style={{display:"flex",gap:8,marginBottom:10}}><ExpPill label="Registration" dateStr={truck.regExpiry}/><ExpPill label="State Inspection" dateStr={truck.inspExpiry}/></div>
<div style={{fontSize:11,color:"#555",fontFamily:"'DM Mono',monospace",marginBottom:10}}>👤 {t.manager} · {t.phone}</div>
{truck.notes&&<div style={{fontSize:11,color:"#3a4455",fontFamily:"'DM Mono',monospace",marginBottom:10,fontStyle:"italic"}}>{truck.notes}</div>}
<div style={{display:"flex",gap:6,marginTop:6}}>
<button onClick={()=>onEdit(truck)} style={{...B("ghost"),padding:"5px 12px",fontSize:12}}>Edit</button>
<button onClick={()=>onDelete(truck.id)} style={{...B("ghost"),padding:"5px 8px",marginLeft:"auto",color:"#ff4444"}}><Ico n="trash" s={13}/></button>
</div>
...
<div style={{fontSize:10,color:"#aa4444",fontFamily:"'DM Mono',monospace",letterSpacing:1,textTransform:"uppercase",marginBottom:4}}>Report Subject</div>
<div style={{fontSize:14,color:"#ff8888",fontFamily:"'DM Mono',monospace",fontWeight:700}}>WORK RELATED INJURY - {form.employeeName?form.employeeName.toUpperCase():"[EMPLOYEE NAME]"}</div>
</div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 14px"}}>
<Field label="Terminal Location"><select style={IS} value={form.terminal} onChange={e=>set("terminal",e.target.value)}>{TERMINALS.map(t=><option key={t} value={t}>{t}</option>)}</select></Field>
...
<div style={{marginBottom:14}}>
<div style={{fontSize:10,color:"#555",fontFamily:"'DM Mono',monospace",letterSpacing:1.2,marginBottom:8,textTransform:"uppercase"}}>Attach Photos, Videos, Medical Documents</div>
<label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",background:"#12121e",border:"2px dashed #2a2a4a",borderRadius:8,padding:"14px 16px",marginBottom:10}}>
<Ico n="attach" s={16}/><span style={{fontSize:13,color:"#666",fontFamily:"'DM Mono',monospace"}}>Click to attach files (images, PDFs, videos)</span>
...
{attachments.map(a=><div key={a.id} style={{display:"flex",alignItems:"center",gap:10,background:"#10101e",border:"1px solid #1e1e30",borderRadius:6,padding:"8px 12px"}}>
<span style={{fontSize:18}}>{a.type.startsWith("image")?"🖼️":a.type.startsWith("video")?"🎬":"📄"}</span>
<div style={{flex:1,minWidth:0}}><div style={{fontSize:12,color:"#ccc",fontFamily:"'DM Mono',monospace",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.name}</div><div style={{fontSize:10,color:"#555",fontFamily:"'DM Mono',monospace"}}>{fmtSize(a.size)}</div></div>
{a.type.startsWith("image")&&<img src={a.data} alt={a.name} style={{width:40,height:40,objectFit:"cover",borderRadius:4,border:"1px solid #2a2a3a"}}/>}
<button onClick={()=>setAttachments(p=>p.filter(x=>x.id!==a.id))} style={{background:"none",border:"none",color:"#ff4444",cursor:"pointer",padding:4}}><Ico n="x" s={13}/></button>
</div>)}
</div>}
</div>
<div style={{display:"flex",gap:8,justifyContent:"flex-end",marginTop:10}}>
...
return <div style={{background:"#0b0b18",border:"1px solid #5a1a1a",borderRadius:10,padding:16}}>
<div style={{background:"#1a0707",border:"1px solid #6a1a1a",borderRadius:7,padding:"8px 12px",marginBottom:12}}>
<div style={{fontSize:10,color:"#aa4444",fontFamily:"'DM Mono',monospace",letterSpacing:1,textTransform:"uppercase",marginBottom:2}}>Work Related Injury Report</div>
<div style={{fontSize:15,fontWeight:800,color:"#ff8888",fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:.5}}>{report.employeeName}</div>
</div>
<div style={{fontSize:12,color:"#666",fontFamily:"'DM Mono',monospace",display:"flex",flexDirection:"column",gap:3,marginBottom:10}}>
<div style={{color:"#4488aa"}}>📍 {report.terminal}</div>
<div style={{color:"#2a3a45",paddingLeft:18}}>{t.address}</div>
...
<div>🩹 <span style={{color:"#cc8888"}}>{report.bodyPart}</span></div>
...
{report.description&&<div style={{background:"#0e0e1c",borderRadius:6,padding:"8px 10px",marginBottom:10,fontSize:12,color:"#888",fontFamily:"'DM Mono',monospace",lineHeight:1.6}}>{report.description.length>120?report.description.slice(0,120)+"...":report.description}</div>}
{report.attachments?.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
{report.attachments.slice(0,4).map((a,i)=>a.type.startsWith("image")?<img key={i} src={a.data} alt={a.name} style={{width:48,height:48,objectFit:"cover",borderRadius:5,border:"1px solid #2a2a3a"}}/>:<div key={i} style={{width:48,height:48,background:"#1a1a2e",border:"1px solid #2a2a3a",borderRadius:5,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20}}>{a.type.startsWith("video")?"🎬":"📄"}</div>)}
{report.attachments.length>4&&<div style={{width:48,height:48,background:"#1a1a2e",border:"1px solid #2a2a3a",borderRadius:5,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#555",fontFamily:"'DM Mono',monospace"}}>+{report.attachments.length-4}</div>}
</div>}
<div style={{fontSize:10,color:"#2a2a3a",fontFamily:"'DM Mono',monospace",marginBottom:10}}>Filed {new Date(report.createdAt).toLocaleDateString()} · By: {report.reportedBy||t.manager}</div>
<div style={{display:"flex",gap:6}}>
...
<button onClick={()=>onDelete(report.id)} style={{...B("ghost"),padding:"5px 8px",marginLeft:"auto",color:"#ff4444"}}><Ico n="trash" s={13}/></button>
</div>
...
const R=({label,value})=>value?<div style={{marginBottom:12}}><div style={{fontSize:10,color:"#555",fontFamily:"'DM Mono',monospace",letterSpacing:1.2,textTransform:"uppercase",marginBottom:3}}>{label}</div><div style={{fontSize:14,color:"#ccc",fontFamily:"'DM Mono',monospace",lineHeight:1.6}}>{value}</div></div>:null;
return <Modal title="Full Injury Report" onClose={onClose} wide>
<div style={{background:"#1a0707",border:"1px solid #8a2020",borderRadius:8,padding:"12px 16px",marginBottom:18}}>
<div style={{fontSize:11,color:"#aa4444",fontFamily:"'DM Mono',monospace",letterSpacing:1.5,marginBottom:4}}>SUBJECT</div>
<div style={{fontSize:17,fontWeight:800,color:"#ff8888",fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:1}}>WORK RELATED INJURY - {report.employeeName?.toUpperCase()}</div>
</div>
...
<div style={{fontSize:10,color:"#555",fontFamily:"'DM Mono',monospace",letterSpacing:1.2,textTransform:"uppercase",marginBottom:3}}>Description of What Happened</div>
<div style={{fontSize:13,color:"#ccc",fontFamily:"'DM Mono',monospace",lineHeight:1.75,background:"#0e0e1c",border:"1px solid #1e1e30",borderRadius:6,padding:"10px 14px"}}>{report.description||"-"}</div>
</div>
<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 20px"}}>
...
{report.attachments?.length>0&&<div style={{marginBottom:14}}>
<div style={{fontSize:10,color:"#555",fontFamily:"'DM Mono',monospace",letterSpacing:1.2,textTransform:"uppercase",marginBottom:8}}>Attachments ({report.attachments.length})</div>
<div style={{display:"flex",flexWrap:"wrap",gap:8}}>
{report.attachments.map((a,i)=><div key={i} onClick={()=>a.type.startsWith("image")&&setLb(a)} style={{cursor:a.type.startsWith("image")?"pointer":"default"}}>
{a.type.startsWith("image")?<img src={a.data} alt={a.name} style={{width:80,height:80,objectFit:"cover",borderRadius:6,border:"1px solid #2a2a3a"}}/>
:<a href={a.data} download={a.name} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,background:"#12121e",border:"1px solid #2a2a3a",borderRadius:6,padding:"10px 14px",textDecoration:"none"}}><span style={{fontSize:28}}>{a.type.startsWith("video")?"🎬":"📄"}</span><span style={{fontSize:10,color:"#666",fontFamily:"'DM Mono',monospace",maxWidth:80,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{a.name}</span></a>}
</div>)}
</div>
</div>}
<div style={{fontSize:11,color:"#2a2a3a",fontFamily:"'DM Mono',monospace"}}>Filed: {new Date(report.createdAt).toLocaleString()}</div>
{lb&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setLb(null)}>
...
const addBtnVariant=tab==="inj"?"danger":"primary";

return <div style={{minHeight:"100vh",background:"#080812",fontFamily:"'Barlow Condensed',sans-serif",color:"#e0e0f0"}}>
<style>{`@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@400;600;700;800&family=DM+Mono:wght@400;500&display=swap'); *{box-sizing:border-box} input:focus,select:focus,textarea:focus{border-color:#ff6200!important;outline:none} ::-webkit-scrollbar{width:5px} ::-webkit-scrollbar-track{background:#0b0b18} ::-webkit-scrollbar-thumb{background:#252538;border-radius:3px} button:hover{opacity:.82} @keyframes slideIn{from{transform:translateX(28px);opacity:0}to{transform:none;opacity:1}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:.45}}`}</style>

```
{/* HEADER */}
<div style={{background:"#09091a",borderBottom:"1px solid #1a1a2e",padding:"0 24px"}}>
  <div style={{maxWidth:1200,margin:"0 auto"}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 0 10px",flexWrap:"wrap",gap:10}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{background:"#ff6200",borderRadius:7,padding:"7px 10px",display:"flex"}}><Ico n="truck" s={20}/></div>
        <div>
          <div style={{fontSize:22,fontWeight:800,letterSpacing:1.2,lineHeight:1}}>PND LOGISTICS MANAGEMENT</div>
          <div style={{fontSize:10,color:"#2a2a44",fontFamily:"'DM Mono',monospace",letterSpacing:1.2,marginTop:2}}>ROAD TESTS · UNIFORMS · FLEET · INJURY REPORTS · 6 TERMINALS</div>
        </div>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        {pendingOut>0&&<div style={{background:"#2a1200",border:"1px solid #ff6200",borderRadius:6,padding:"5px 10px",display:"flex",alignItems:"center",gap:5,animation:"pulse 2s infinite"}}><Ico n="bell" s={13}/><span style={{fontSize:12,color:"#ff9944",fontFamily:"'DM Mono',monospace"}}>{pendingOut} awaiting outcome</span></div>}
        ...
        {lastSync&&<span style={{fontSize:10,color:"#1e1e3a",fontFamily:"'DM Mono',monospace"}}>{lastSync.toLocaleTimeString()}</span>}
      </div>
    </div>
    <div style={{display:"flex"}}>
      {TABS.map(t=><button key={t.key} onClick={()=>setTab(t.key)} style={{background:"none",border:"none",borderBottom:`2px solid ${tab===t.key?"#ff6200":"transparent"}`,color:tab===t.key?"#ff6200":"#3a3a5a",cursor:"pointer",padding:"10px 20px",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700,fontSize:14,letterSpacing:1,display:"flex",alignItems:"center",gap:7,transition:"all .2s"}}>
        <Ico n={t.icon} s={14}/>{t.label}
        {t.count>0&&<span style={{background:t.badgeColor,color:"#fff",borderRadius:99,padding:"1px 7px",fontSize:10}}>{t.count}</span>}
      </button>)}
    </div>
  </div>
</div>
...
{/* STATS */}
<div style={{background:"#09091a",borderBottom:"1px solid #14142a",padding:"10px 24px"}}>
  <div style={{maxWidth:1200,margin:"0 auto",display:"flex",gap:24,flexWrap:"wrap"}}>
    {[{l:"Total Tests",v:rts.length,c:"#444"},{l:"Scheduled",v:rts.filter(r=>r.status==="Scheduled").length,c:"#4db8ff"},{l:"Passed",v:rts.filter(r=>r.status==="Passed").length,c:"#00ee77"},{l:"Failed",v:rts.filter(r=>r.status==="Failed").length,c:"#ff5555"},{l:"Pending Orders",v:unis.filter(u=>u.status==="Pending").length,c:"#ffcc44"},{l:"Fleet Alerts",v:truckAlerts,c:"#ffaa00"},{l:"Total Trucks",v:trucks.length,c:"#666"},{l:"Injury Reports",v:injs.length,c:"#ff6666"}].map(s=><div key={s.l} style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontFamily:"'DM Mono',monospace",fontSize:10,color:"#2a2a44",textTransform:"uppercase",letterSpacing:1}}>{s.l}</span><span style={{fontFamily:"'Barlow Condensed',sans-serif",fontWeight:800,fontSize:22,color:s.c,lineHeight:1}}>{s.v}</span></div>)}
  </div>
</div>
...
<div style={{maxWidth:1200,margin:"0 auto",padding:"22px 24px"}}>
  <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap",alignItems:"center"}}>
    <select style={{...IS,width:"auto",minWidth:240}} value={fTerm} onChange={e=>setFTerm(e.target.value)}>
    ...
    <button onClick={()=>setModal({type:addType[tab]})} style={{...B(addBtnVariant),display:"flex",alignItems:"center",gap:6,marginLeft:"auto"}}>
      <Ico n="plus" s={14}/>{addLabel[tab]}
    </button>
  </div>

  {loading ? (
    <div style={{textAlign:"center",padding:80,color:"#1e1e3a",fontFamily:"'DM Mono',monospace"}}>Loading shared data...</div>
  ) : tab==="rt" ? (
    fRts.length===0
      ? <div style={{textAlign:"center",padding:80,color:"#1e1e3a",fontFamily:"'DM Mono',monospace",fontSize:13}}>No road tests yet. Click "Schedule Road Test" to begin.</div>
      : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:14}}>
          {fRts.map(t=><RTCard key={t.id} test={t} onEdit={t=>setModal({type:"editRT",data:t})} onOutcome={t=>setModal({type:"outcome",data:t})} onDelete={delRT} onSms={t=>setModal({type:"sms",data:t})}/>)}
        </div>
  ) : tab==="uni" ? (
    fUnis.length===0
      ? <div style={{textAlign:"center",padding:80,color:"#1e1e3a",fontFamily:"'DM Mono',monospace",fontSize:13}}>No uniform requests yet.</div>
      : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:14}}>
          {fUnis.map(u=><UniCard key={u.id} req={u} onEdit={r=>setModal({type:"editUni",data:r})} onDelete={delUni} onFulfill={fulfillUni}/>)}
        </div>
  ) : tab==="fleet" ? (
    <>
      {truckAlerts>0&&<div style={{background:"#1e1000",border:"1px solid #7a4400",borderRadius:8,padding:"10px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:10}}><Ico n="warn" s={16}/><span style={{fontSize:14,color:"#ffcc44",fontFamily:"'Barlow Condensed',sans-serif",fontWeight:700}}>{truckAlerts} truck{truckAlerts>1?"s":""} require attention - registration or inspection expiring soon / expired</span></div>}
      {fTrucks.length===0
        ? <div style={{textAlign:"center",padding:80,color:"#1e1e3a",fontFamily:"'DM Mono',monospace",fontSize:13}}>No trucks yet. Click "Add Truck" to begin.</div>
        : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
            {fTrucks.map(t=><TruckCard key={t.id} truck={t} onEdit={t=>setModal({type:"editTruck",data:t})} onDelete={delTruck}/>)}
          </div>
      }
    </>
  ) : (
    fInjs.length===0
      ? <div style={{textAlign:"center",padding:80,color:"#1e1e3a",fontFamily:"'DM Mono',monospace",fontSize:13}}>No injury reports filed yet. Click "File Injury Report" to begin.</div>
      : <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(340px,1fr))",gap:14}}>
          {fInjs.map(r=><InjuryCard key={r.id} report={r} onView={r=>setModal({type:"viewInj",data:r})} onEdit={r=>setModal({type:"editInj",data:r})} onDelete={delInj}/>)}
        </div>
  )}
</div>
